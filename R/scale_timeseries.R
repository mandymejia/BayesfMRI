...
scale_timeseries <- function(BOLD, scale=TRUE, transpose = TRUE){
	...
	if( min_local_mean < 1) warning("Scaling to percent signal change when locations have means less than 1 may cause errors or produce aberrant results.")
	if( min_local_mean < 0.1) stop("Some local means are less than 0.1. Please set scale_BOLD = FALSE.")
