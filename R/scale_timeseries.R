...
#' @param scale Option for scaling units to percent local signal change AND
#' 	centering. If false, only centers.
...
scale_timeseries <- function(BOLD, scale=TRUE, transpose = TRUE){
	...
	min_local_mean <- min(local_means, na.rm = T)
	if( min_local_mean < 1 & scale) warning("Scaling to percent signal change when locations have means less than 1 may cause errors or produce aberrant results.")
	if( min_local_mean < 0.1 & scale) stop("Some local means are less than 0.1. Please set `scale_BOLD = FALSE` or set `meanTol` > .1 in the call to BayesGLM.")
