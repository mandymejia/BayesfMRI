find.connected <- function(ind,mesh){
  ...
  while(length(ind.add)>0){
    set.k <- c(ind.add[1])
    ind.add <- ind.add[-1]
    #chech which other nodes that are connected to the first
...
# Function that is used to find the indices in the mesh corresponding to the smallest
# excursion set. NULL is returned if all sets have areas larger than area.limit.
# if res is is of class excurobj, continuous is not used, and instead the function
# uses the approximation that the contribution from each element \phi_i is 
# \int \phi_i(s)ds = C_ii. 
...
# Function similar to the excursions function, but which also takes a mesh as input and
# computes modified excursion sets where all regions with areas smaller than area.limit
# are removed in the internal calculations.
...
excursions.no.spurious <- function(alpha,
                                   ...
                                   factor = 1, #1/2 for removing area.limit around each point
                                   type,
                                   ind,
                                   n.iter=10000,
                                   Q.chol,
