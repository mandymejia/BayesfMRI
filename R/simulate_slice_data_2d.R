...
#'
#' @return A list with elements \code{BOLD} (a list of arrays), \code{design}
#'   (a list of matrices), and \code{betas}. The list \code{betas} is a list
...
#'
#' @examples
#' data <- simulate_slice_data_2d()
simulate_slice_data_2d <-
  function(num_sessions = 1,
    ...
    y_i <- sapply(seq(num_sessions), function(i) {
      ...
      y_t <- aperm(y_t,c(2,3,1))
     # Return everything
      return(list(BOLD = y_t,design = tasks, betas = beta_coefficients))
    }, simplify = F)
    names(y_i) <- paste("session",seq(num_sessions), sep ="_")
    BOLD <- sapply(y_i, `[[`, i = "BOLD", simplify = F)
    design <- sapply(y_i, `[[`, i = "design", simplify = F)
    betas <- sapply(y_i, `[[`, i = "betas", simplify = F)
    return(list(BOLD = BOLD, design = design, betas = betas))
  }
