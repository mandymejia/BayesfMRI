...
BayesGLM <- function(
 ...
 # if (nS == 1 && combine_sessions) combine_sessions <- FALSE
  ...
  if(is.null(design_multiple)){
    ...
    # Step 1: Identify no-signal locations. Compare null vs. canonical model using F test.

    # F = (RSS0 - RSS1)/RSS1 * (n - p1)/(p1 - p0), where model 0 = null model & model 1 = canonical HRF model.  So p1 - p0 = nK (# of task regressors).
    ...
    XtX_inv1 <- try(Matrix::solve(Matrix::crossprod(X1_ss)))
    XtX_inv0 <- try(Matrix::solve(Matrix::crossprod(X0_ss)))
    ...
    coef1 <- XtX_inv1 %*% t(X1_ss) %*% y_ss
    coef0 <- XtX_inv0 %*% t(X0_ss) %*% y_ss
    resid1 <- y_ss - X1_ss %*% coef1 #TxV matrix
    resid0 <- y_ss - X0_ss %*% coef0 #TxV matrix
    RSS1 <- colSums(resid1^2)
    RSS0 <- colSums(resid0^2)

    #compute F-statistic (step c)
    DOF1 <- nT - ncol(X1_ss) #will be over-estimated, but shouldn't matter much because it's essentially Inf
    Fstat[mask==TRUE] <- (RSS0 - RSS1)/RSS1 * DOF1/nK
    pval_F[mask==TRUE] <- 1 - pf(Fstat[mask==TRUE], df1 = nK, df2 = DOF1)
    #pval_Chi2[mask==TRUE] <- 1 - pchisq(nK*Fstat[mask==TRUE], df = nK)
