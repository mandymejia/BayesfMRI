...
#' @param correction  Type of multiple comparisons correction, 'FDR' (Benjamini Hochberg) or 'FWER' (Bonferroni correction)
#' @param alpha Significance level (e.g. 0.05)
#'
#' @return a list containing...
#' @export
classicalGLM2 <- function(
  results,
  brainstructure = NULL,
  session_name = NULL,
  gamma = 0,
  ...
  if(length(results_class) > 1) stop("All elements in results should have class
  ...
  brainstructures <- names(first_result)[which(!sapply(first_result, is.null))]
  if(is.null(brainstructure)) {
    message(paste0("No brainstructure specified. Using first non-null brainstructure in the object, which is ",brainstructures[1],".\n"))
  ...
  if(is.null(session_name)){
    if(has_avg) {
      message("No session specified, but the session named avg, denoting an average across sessions, is present and will be used.")
      session_name <- "avg"
    } else {
      message(paste0("No session specified, and avg is not present. The first available session, ",sess_names[1],", will be used"))
      session_name <- sess_names[1]
    }
  }
  if(results_class == "character"){
    results <- sapply(results, function(x) readRDS(x)$GLMs_classical[[brainstructure]][[session_name]]$estimates, simplify = "array")
  }
  if(results_class == "BayesGLM_cifti") {
    results <- sapply(results, function(x) x$GLMs_classical[[brainstructure]][[session_name]]$estimates, simplify = "array")
  }
  avg_estimate <- apply(results,1:2,mean)

  gamma_result <- sapply(gamma, function(g) {
    p_vals <- apply(results,1:2, function(x) t.test(x,alternative = "greater", mu = g)$p.value)
    p_vals_adj <- apply(p_vals,2,p.adjust, method = p_method)
