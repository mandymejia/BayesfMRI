...
This vignette is designed to make you familiar with the process of using the `BayesfMRI` package to perform analysis on cortical surface fMRI data.
...
od <- organize_data(y = simulated_data$data$session_1$BOLD,
                    X = simulated_data$data$session_1$design)
y <- od$y
...
system.time(Sig_inv <- Q + crossprod(X%*%Psi))
  #  user  system elapsed 
  # 0.068   0.015   0.083
m <- t(X%*%Psi)%*%y / sigma2
mu <- solve(Sig_inv,m)
...
system.time(Q_chol_inv <- solve(Q_chol))
  #  user  system elapsed 
  # 0.135   0.018   0.154 
A <- crossprod(X%*%Psi)/sigma2
system.time(Sigma <- Q_chol_inv %*% (A + diag(1,length(diag(A)))) %*% t(Q_chol_inv))
  #  user  system elapsed 
  # 3.172   0.120   3.291
sigma2_hat <- crossprod(y) - 2*crossprod(y,X%*%Psi%*%mu) + crossprod(X%*%Psi%*%mu) + sum(colSums(A*Sigma))
