...
In the pursuit of open access goals, brain tempate data were taken from FSL, rather than SPM8, as is used in [@mejia2020bayesian], and the binary mask image used is $45 \times 54$ (rather than $46 \times 55$). Using tools from the `neuRosim` package, the simulated data are recreated below:
...
```{r sim data plot, message=F, warning=F, echo=F}
...
```{r Data generation for group-level analysis}
# Load the binary template image
data(binary_template)
...
y_i <- sapply(seq(sample_size), function(i) {
...
## Bayesian GE Analysis
```{r}
y_ge <- Reduce(rbind,y_i)
X_ge <- cbind(rep(t1,10),rep(t2,10))
# Make a session list (for INLA)
session <- list(
  BOLD = y_ge,
  design = X_ge
...
# Run the Bayes
GE_result <- BayesGLM_surface(data = data, mesh = mesh, verbose = F)
```

## Bayesian FE Analysis
