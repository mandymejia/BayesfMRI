...
Due to my familiarity with FSL over SPM8, I have created a binary mask image that is $45 \times 54$ (rather than $46 \times 55$, as in [@mejia2020bayesian]). Using tools from the `neuRosim` package, the data are recreated below:

```{r show template image activation profile and amplitude}
# Load the binary template image
binary_template <- readRDS("binary_image_slice.rds") # This needs to be changed.
...
```{r Classical GLM}
single_subject_classical <- classicalGLM(data = data, scale = T)
