...
```{r setup}
# Replace '~/pardiso.lic' with the actual path to the pardiso license on your computer
library(INLA)
...
plot_slice <- function(X, color_palette = NULL, zlim = NULL) {
  ...
  if(is.null(zlim)) {
    max_abs <- max(abs(reshape2::melt(X)$value), na.rm = T)
...
plot_slice(multi_data$betas$session_1, zlim=c(0,1.5))
```

Session 2: 

```{r True Activation Amplitudes 2, fig.height=3, fig.width=7, echo=F}
plot_slice(multi_data$betas$session_2, zlim=c(0,1.5))
```
...
multi_session_result <- BayesGLM(data=multi_data$data, mesh=mesh, scale_BOLD = FALSE, scale_design = FALSE, verbose = FALSE, return_INLA_result=TRUE, num.threads = parallel::detectCores() - 2, avg_betas_over_sessions = T)
saveRDS(multi_session_result, "/Volumes/GoogleDrive/My Drive/BayesfMRI/scratch/BayesGLM_multisession_linear_combinations.rds")
```
...
```{r}
average_betas <- function(BayesGLM_obj) {
  n_sess <- length(BayesGLM_obj$beta_estimates)
  avg_betas <- Reduce(`+`,BayesGLM_obj$beta_estimates)/n_sess
  colnames(avg_betas) <- BayesGLM_obj$beta_names
  return(avg_betas)
}

convert_verts_to_locs <- function(betas_mat, BayesGLM_obj, mask) {
  in_binary_mask <- which(mask == 1, arr.ind = T)
  in_binary_mask <- in_binary_mask[,2:1]
  trans_mat <- inla.spde.make.A(mesh = BayesGLM_obj$mesh, loc = in_binary_mask)
  locs_betas <- as.matrix(trans_mat %*% betas_mat)
  locs_betas_in_mask <- sapply(split(locs_betas, col(locs_betas)), function(lb, mask) {
    out <- mask
    out[out == 1] <- lb
    out[mask == 0] <- NA
    return(out)
  }, mask = mask, simplify = F)
  return(locs_betas_in_mask)
}

plot_image_2D(locs_betas_in_mask)
```
