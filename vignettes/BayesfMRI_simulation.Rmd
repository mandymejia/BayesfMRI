...
reshape2::melt(classical_bbeta) %>% 
  ...
  scale_fill_gradientn("",colors = wb_palette$color, 
                       values = wb_palette$value, limits = c(-1,1), 
                       na.value = "white") +
  labs(x="",y="") +
  facet_grid(.~L1) +
  theme_bw() +
...
plot_BayesGLM_2d(single_subject_result, mask = binary_template, zlim = c(-1,1))
```
...
plot_BayesGLM_2d(multi_session_result, mask = binary_template, zlim = c(-1.76,1.76))
```
...
combined_session_results <- BayesGLM_surface(data = multi_session_result, mesh = mesh, contrasts = contrasts_def, verbose = F)
```
