...
library(BayesfMRI) 
library(ciftiTools)
ciftiTools.setOption('wb_path','~/Desktop/workbench') #where your Connectome Workbench installation is located
```
...
system.time(
  bglm <- BayesGLM(BOLD=BOLD_xii,
        ...
        brainstructures=c("left", "sub"), #cortical surfaces and subcortex
        subROI=c('Amygdala-L'),
        TR=TR,
        nuisance=motion,
        scale_BOLD='mean',
...
act2 <- activations(bglm2, gamma=0)
q <- prevalence(list(act, act2))
```
...
sesssions separately, then passed those results into `BayesGLM2()`. By default, 
...
```{r, rgl=TRUE, format = "jpg"}
plot(bglm2, idx=1, title = "Group-average (fear)")
```
