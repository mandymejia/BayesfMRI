...
plot(prev$prev_test_xii$single_sess)
```
...
The design matrix can vary across the data locations. For example, spatially-varied HRF modeling will yield a different design matrix for each data location, even if the task design is unchan For spatially-varying design, the `design` argument will be a 3-dimensional array.

In this example, we will just use a duplciated, identical design matrix for each location.
...
```{r, rgl=TRUE, format = "jpg", fig.width=3.5}
