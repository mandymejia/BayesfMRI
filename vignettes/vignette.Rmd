...
**Multi-session/Longitudinal Modeling**: 
...
Let's display the group-average estimates and areas of activation.
...
```{r, rgl=TRUE, format = "jpg"}
plot(bglm2, idx=1, title = "Group-average (fear)", what = 'activation')
```
...
task. Historically prevalence mapping has been limited by low power in first-level 
modeling.  Spatial Bayesian modeling has much higher power to detect activations
at the subject level, facilitating prevalence mapping.
...
`gamma = 0` (no minimum effect size). We will read in the first-level activation 
