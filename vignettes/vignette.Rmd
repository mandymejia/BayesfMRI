...
We will start by reading in each of these. Note that all of the data we will use can be found in the Github repository for `BayesfMRI` at [https://github.com/mandymejia/BayesfMRI/].
...
be useful to set $\gamma$ to a higher value to identify only activations exeeding a 
...
```{r}
# Point to the results of BayesGLM for several subjects
#bglm_fnames <- list.files("../vignette_data/glm_results")
#bglm_fnames <- file.path("../vignette_data/glm_results", bglm_fnames)

# Run BayesGLM2 to do second-level analysis
#bglm2 <- BayesGLM2(bglm_fnames, gamma = 0, alpha = 0.05, excursion_type = '>', num_cores = 8)
...
```{r, rgl=TRUE, format = "jpg"}
plot(bglm2, type = 'contrasts', idx=1, title = "Group-average (fear)", what = 'surface')
```

```{r, fig.height=4, fig.width=4, fig.show='hold'}
slices <- 27:30 
plot(bglm2, type = 'contrasts', idx="fear", title = "fear", what = 'volume', slices = slices, n_slices=4, zlim = c(-0.5,0.5)) 
cat('\t')
plot(bglm2, type = 'contrasts', idx="neut", title = "neutral", what = 'volume', slices = slices, n_slices=4, zlim = c(-0.5,0.5)) 
```

And the activation maps:

### [TO DO] Show surface results too

```{r, fig.height=4, fig.width=4, fig.show='hold'}
slices <- 27:30 
plot(bglm2, type = 'activations', idx="fear", title = "fear", what = 'volume', slices = slices, n_slices=4)
cat('\t')
plot(bglm2, type = 'activations', idx="neut", title = "neutral", what = 'volume', slices = slices, n_slices=4, zlim = c(-0.5,0.5)) 
```
...
#### [TO DO] Fill this part in with additional information and example

<!-- **Scrubbing**: To remove certain time points due to head motion or other reasons (e.g. drowsiness), the `scrub` argument will be used to indicate any time points that should be excluded from analysis. Using a simultaneous regression approach, time points will only be removed after the DCT bases are generated and the prewhitening parameters are estimated, since those steps assume the original temporal structure of the data. -->
...
#### [TO DO] Fill this part in with additional information and example, including how this affects BayesGLM2 and prevalence mapping
...
**Prevalence Map testing**: Inference on prevalence maps to determine where a certain proportion of participants (e.g. at least 50\%) display activation, or where there are differences in the prevalence of activation between groups.
