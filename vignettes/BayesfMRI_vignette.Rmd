...
We can also visualize the estimates masked by the corresponding activations using the `act_mask` argument to the estimates plotting function. Here's that on the left cortex for the spatial GLM.

```{r, rgl=TRUE, format = "jpg", fig.height=4.2, fig.width=2.5}
plot(
  bglm, Bayes=TRUE, idx="fear", title = "spatial GLM (fear), masked", 
  act_mask=act, what = 'surface', hemisphere="left"
)
```
...
```{r}
fnames <- list.files("glms_full")
# bglm2b <- BayesGLM2(file.path("glms_full", fnames), excursion_type = '>', num_cores = 20, contrast_list = list(fear_vs_neut = rep(c(1, -1), 10)/10))
